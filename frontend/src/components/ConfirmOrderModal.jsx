...
export const ConfirmOrderModal = () => {
  const { productsSelected, addProductsSelectedAction, clearProductsSelectedAction, deleteProductSelecteddAction, updateProductSelecteddAction } = useOrderStore()
  ...
  const GlobalSubtotal = (productsSelectedLocal.reduce((acumulador, producto) => {
    const subtotal = producto.quantity * producto.price; // Calcula el subtotal de cada producto
    return acumulador + subtotal; // Suma el subtotal al total acumulado
  }, 0)); 
      ...
      <div className='fixed inset-0 bg-black bg-opacity-30 backdrop:blur-sm flex justify-center items-center'>
        <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
          <div className="max-w-2xl w-full bg-white rounded-xl shadow-lg p-6">
            ...
            <button onClick={()=>clearProductsSelectedAction()} className="w-full bg-warning/70 hover:bg-warning text-white font-medium py-3 rounded-lg transition-colors">
              Cancelar
            </button>
            </div>
          </div>
